...
// --- OAuth: authorization_code ---
{
  ...
  check(
    "oauth: valid auth code -> access+refresh",
    !!good.access_token && !!good.refresh_token,
  );
  ...
  const ug = await sim.request("POST", "/oauth/token", {
...
// --- refresh rotation: old token invalidated atomically ---
{
  ...
  check(
    ...
    !!t2.access_token && t2.access_token !== t1.access_token,
  );
...
// --- clinics ---
{
  const { sim } = mkSim();
  const r = await j(
    await sim.request("GET", "/api/v1/clinics", { headers: H }),
  );
  check(
    "clinics: returns sandbox clinics",
    r.data.clinics.length === SANDBOX_CLINICS.length,
  );
  check(
    "clinics: all fixtures simulated",
    r.data.clinics.every((c: any) => c.simulated === true),
  );
}
...
{
  const { sim } = mkSim();
  const noFilter = await sim.request("GET", "/api/v1/appointments", {
    headers: H,
  });
  check("appointments GET: no filter -> 422", noFilter.status === 422);
  const withFilter = await sim.request(
    "GET",
    "/api/v1/appointments?filter[clinic_id]=1",
    { headers: H },
  );
  check(
    "appointments GET: filter[clinic_id] -> 200",
    withFilter.status === 200,
  );
}
...
{
  const { sim } = mkSim();
  const noArgs = await sim.request("GET", "/api/v1/vets-timeslots", {
    headers: H,
  });
  check("timeslots: no visit_type_id/duration -> 422", noArgs.status === 422);
  const ok = await sim.request(
    "GET",
    "/api/v1/vets-timeslots?visit_type_id=5",
    { headers: H },
  );
  check("timeslots: visit_type_id -> 200", ok.status === 200);
  const missing = await sim.request(
    "GET",
    "/api/v1/vets-timeslots?visit_type_id=5&clinic_id=9999",
    { headers: H },
  );
...
// --- pets ---
{
  ...
  check(
    "pets: missing fields -> 422 with field list",
...
// --- appointments POST ---
{
  ...
  check(
    "appointments POST: reminder_notifications required -> 422",
    noReminders.status === 422,
  );
  ...
  check(
    "appointments POST: duplicate client_reference -> 409",
    dup.status === 409,
  );
...
// --- cancel ---
{
  ...
  const missing = await sim.request(
    "POST",
    "/api/v1/appointments/424242/cancel",
    { headers: H },
  );
  check("cancel: unknown appointment -> 404", missing.status === 404);
  const ok = await j(
    await sim.request("POST", "/api/v1/appointments/1000/cancel", {
      headers: H,
    }),
  );
...
// --- failure modes ---
{
  ...
  check(
    "failmode: malformed (non-json 200)",
    rm.status === 200 && !ct.includes("json"),
  );
  ...
  check(
    "failmode: timeout aborted by caller",
    timeoutRace === "caller-aborted",
  );
  state.failMode = null;
}
...
// --- 20-way concurrent confirmation: 1 winner, 19 conflicts ---
{
  ...
  for (const [, a] of state.appointments)
    if (a.clientReference === "race-ref") stored++;
  check("concurrency: appointment created exactly once", stored === 2); // ref key + id key
}

// --- circuit breaker lifecycle ---
{
  const breaker = createSimulatedBreaker({
    failureThreshold: 3,
    resetAfterMs: 40,
  });
  check(
    "breaker: starts closed",
    breaker.state() === "closed" && breaker.canTry(),
  );
  breaker.onFailure();
  breaker.onFailure();
  ...
  check(
    ...
    breaker.state() === "open" && !breaker.canTry(),
  );
  await new Promise((r) => setTimeout(r, 60));
  const canTryNow = breaker.canTry();
  check(
    "breaker: half-open after reset window",
    breaker.state() === "half-open" && canTryNow,
  );
  breaker.onFailure();
  check(
    "breaker: half-open failure re-opens",
    breaker.state() === "open" && !breaker.canTry(),
  );
  await new Promise((r) => setTimeout(r, 60));
  breaker.canTry();
  breaker.onSuccess();
  check(
    "breaker: half-open success closes",
    breaker.state() === "closed" && breaker.canTry(),
  );
}
