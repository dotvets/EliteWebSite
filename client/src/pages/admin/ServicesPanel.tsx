...
export default function ServicesPanel({
  ...
  const toggle = async (s: Service) => {
    await api(`/api/admin/services/${s.id}`, {
      ...
      body: JSON.stringify({
        ...s,
        isActive: s.isActive === "true" ? "false" : "true",
      }),
    });
    load();
  };
  ...
  const onImportFile = async (file: File) => {
    ...
    try {
      ...
      if (!rows.length) {
        setMsg("❌ الملف فارغ أو غير مقروء");
        setImporting(false);
        return;
      }
      const res = await api("/api/admin/services/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        >
          ...
          />
          <a
            href="/api/admin/services/template"
            style={{
              ...S.btnGhost,
              textDecoration: "none",
      ...
      <div style={S.card}>
          ...
          <tbody>
            {filtered.map((s) => (
              <tr key={s.id}>
                ...
                <td style={S.td}>
                  <span style={s.isActive === "true" ? S.badgeOn : S.badgeOff}>
                    {s.isActive === "true" ? "مفعّلة" : "معطّلة"}
                  </span>
                </td>
                <td style={{ ...S.td, whiteSpace: "nowrap" }}>
                  ...
                  <button style={S.btnGhost} onClick={() => toggle(s)}>
                    {s.isActive === "true" ? "تعطيل" : "تفعيل"}
                  </button>{" "}
                  <button style={S.btnDanger} onClick={() => remove(s)}>
                    حذف
                  </button>
                </td>
              </tr>
        ...
        {filtered.length === 0 && (
          <p style={{ color: "#888" }}>لا خدمات مطابقة.</p>
        )}
      </div>
    </div>
  );
