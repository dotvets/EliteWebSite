      ...
      >
        {toasts.map((t) => (
          <div
            key={t.id}
            style={{
              background: t.kind === "ok" ? "#1a7f37" : "#c0392b",
              color: "#fff",
              padding: "10px 18px",
              borderRadius: 10,
    ...
    <div style={A.overlay} onClick={onNo}>
      <div style={A.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
          <button style={A.btnDanger} onClick={onYes}>
            تأكيد
          </button>
          <button style={A.btnGhost} onClick={onNo}>
            إلغاء
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <div>
          <div style={{ fontSize: 11, color: "#999", marginBottom: 3 }}>
            العربية{" "}
            {!valueAr && (
              <span style={{ color: "#c80" }}>⚠ العربية مفقودة</span>
            )}
          </div>
          <Tag
            style={{
        ...
        <div>
          <div style={{ fontSize: 11, color: "#999", marginBottom: 3 }}>
            English{" "}
            {!valueEn && (
              <span style={{ color: "#c80" }}>⚠ English missing</span>
            )}
          </div>
          <Tag
            style={{
...
export function Empty({ text }: { text: string }) {
  return (
    <div
      style={{ textAlign: "center", color: "#999", padding: 40, fontSize: 15 }}
    >
      {text}
    </div>
  );
