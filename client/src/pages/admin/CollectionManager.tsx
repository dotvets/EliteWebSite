...
export type FieldDef =
  | { key: string; label: string; type: "text" | "textarea" | "number" | "date" }
    ...
    return (
      <div key={f.key} style={{ marginBottom: 12 }}>
        ...
        {f.type === "textarea"
          ...
          : <input style={A.input} type={f.type === "number" ? "number" : f.type === "date" ? "date" : "text"} value={form[f.key] || ""} onChange={(e) => { setForm({ ...form, [f.key]: e.target.value }); setDirty(true); }} />}
      </div>
    );
  };
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id}>
                  ...
                  <td style={A.td}><StatusBadge on={r.published === "true"} onText="منشور" offText="مخفي" /></td>
                  {orderable && (
                    <td style={{ ...S_tdNowrap }}>
