...
const BRANCH_OPTS: [string, string][] = [
  ["الرياض — قرطبة", "الرياض — قرطبة"],
  ["جدة — الحمراء", "جدة — الحمراء"],
];
...
export const TeamPanel = ({ api }: { api: Api }) => (
  <CollectionManager
    api={api}
    route="team"
    title="الفريق / الأطباء"
    orderable
    listCols={[
      { key: "nameAr", label: "الاسم" },
      { key: "positionAr", label: "المنصب" },
    ...
    fields={[
      ...
      {
        key: "publishAt",
        label: "موعد النشر المجدول (اختياري — يظهر تلقائياً في هذا الموعد)",
        type: "datetime",
      },
    ]}
  />
...
export const TestimonialsPanel = ({ api }: { api: Api }) => (
  <CollectionManager
    ...
    orderable
    listCols={[
      { key: "customerAr", label: "العميل" },
      { key: "textAr", label: "الرأي" },
  ...
  <CollectionManager
    ...
    fields={[
      ...
      },
      {
        ...
        label: "الوصف",
        type: "bi",
        bi: ["descriptionAr", "descriptionEn"],
        textarea: true,
      },
  ...
  <CollectionManager
    ...
    fields={[
      ...
      { key: "publishedAt", label: "تاريخ النشر", type: "date" },
      {
        ...
        label: "SEO — العنوان",
        type: "bi",
        bi: ["metaTitleAr", "metaTitleEn"],
      },
      {
        key: "metaDesc",
        label: "SEO — الوصف",
        type: "bi",
        bi: ["metaDescAr", "metaDescEn"],
        textarea: true,
      },
    ]}
  />
...
const SETTING_KEYS: {
...
}[] = [
  {
    group: "عام",
    items: [
      { key: "settings.siteName", label: "اسم الموقع" },
      { key: "settings.logo", label: "الشعار (رابط)" },
      {
        key: "settings.announcement",
        label: "شريط إعلان (اتركه فارغاً لإخفائه)",
      },
    ],
  },
  {
    group: "التواصل",
    items: [
      { key: "settings.phone", label: "الهاتف الموحد" },
      { key: "settings.whatsapp", label: "واتساب" },
      { key: "settings.email", label: "البريد" },
      { key: "settings.address", label: "العنوان" },
    ],
  },
  {
    group: "وسائل التواصل",
    items: [
      { key: "settings.social.facebook", label: "فيسبوك" },
      { key: "settings.social.twitter", label: "تويتر/X" },
      { key: "settings.social.instagram", label: "إنستغرام" },
      { key: "settings.social.snapchat", label: "سناب شات" },
      { key: "settings.social.tiktok", label: "تيك توك" },
    ],
  },
];
    ...
    <div>
      ...
      >
        {SETTING_KEYS.map((s, i) => (
          <button
            key={s.group}
            style={i === group ? A.btn : A.btnGhost}
            onClick={() => setGroup(i)}
          >
            {s.group}
          </button>
      ...
      <div style={A.card}>
        ...
        <p style={{ color: "#888", fontSize: 13 }}>
          هذه القيم مصدر موحد — تغييرها يحدّث الموقع كله تلقائياً.
        </p>
        {g.items.map((it) => (
          <div
            key={it.key}
          ...
          >
            ...
            <input
              ...
              value={vals[it.key] || ""}
              onChange={(e) => setVals({ ...vals, [it.key]: e.target.value })}
            />
            <button style={A.btn} onClick={() => saveKey(it.key)}>
              حفظ
            </button>
          </div>
        ))}
...
export function SeoPanel({ api }: { api: Api }) {
  ...
  const save = async () => {
    await api(`/api/admin/seo${encodeURI(sel)}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const Counter = ({ v, max }: { v: string; max: number }) => (
    <span
      style={{ fontSize: 11, color: (v || "").length > max ? "#c00" : "#999" }}
    >
      {(v || "").length}/{max}
    </span>
  );
  return (
    <div style={A.card}>
      ...
      <div style={{ marginTop: 16 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#6650a0" }}>
          عنوان ميتا (عربي) <Counter v={form.titleAr} max={60} />
        </div>
        <input
          style={A.input}
          value={form.titleAr || ""}
        ...
        />
        <div style={{ fontSize: 13, fontWeight: 700, color: "#6650a0" }}>
          Meta Title (EN) <Counter v={form.titleEn} max={60} />
        </div>
        <input
          style={A.input}
          dir="ltr"
        ...
        />
        <div style={{ fontSize: 13, fontWeight: 700, color: "#6650a0" }}>
          وصف ميتا (عربي) <Counter v={form.descAr} max={160} />
        </div>
        <textarea
          style={{ ...A.input, minHeight: 70 }}
          value={form.descAr || ""}
          onChange={(e) => setForm({ ...form, descAr: e.target.value })}
        />
        <div style={{ fontSize: 13, fontWeight: 700, color: "#6650a0" }}>
          Meta Description (EN) <Counter v={form.descEn} max={160} />
        </div>
        <textarea
          style={{ ...A.input, minHeight: 70 }}
          dir="ltr"
        ...
        <button style={A.btn} onClick={save}>
          حفظ SEO
        </button>
      </div>
    </div>
...
export function MediaPanel({
  ...
  api: Api;
  uploadMedia: (f: File) => Promise<void>;
}) {
  const toast = useToast();
          ...
          {filtered.map((m) => (
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "#aaa" }}>
                {m.size ? Math.round(Number(m.size) / 1024) + "KB" : ""} •{" "}
                {String(m.uploadedAt || "").slice(0, 10)}
              </div>
              <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                ...
                <button style={A.btnDanger} onClick={() => del(m)}>
                  حذف
                </button>
              </div>
            </div>
...
export function GlobalSearch({
  ...
  return (
    <div style={{ position: "relative", margin: "0 24px 16px" }}>
      <input
        style={{ ...A.search, maxWidth: "100%" }}
        placeholder="🔎 بحث شامل في كل المحتوى…"
        value={q}
      ...
      {results.length > 0 && (
        ...
        >
          {results.map((r, i) => (
            ...
            >
              <span style={{ color: "#6650a0", fontWeight: 700, fontSize: 12 }}>
                {r.type}
              </span>{" "}
              — {r.label}
            </div>
...
export function PagesPanel({ api }: { api: Api }) {
  ...
  const orderedKeys = () => {
    const keys = HOME_SECTIONS.filter((s) => s.key !== "hero").map(
      (s) => s.key,
    );
    if (!cfg.order.length) return keys;
    return [...keys].sort((a, b) => {
      const ia = cfg.order.indexOf(a),
        ib = cfg.order.indexOf(b);
  ...
  const save = async () => {
    setSaving(true);
    await api("/api/admin/content/home.sections", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        valueAr: JSON.stringify(cfg),
        type: "json",
        section: "home",
      }),
    });
  ...
  return (
    <div style={A.card}>
      ...
      <p style={{ color: "#888", fontSize: 13 }}>
        إظهار/إخفاء وترتيب أقسام الصفحة الرئيسية. التأثير حقيقي ومباشر على
        الموقع العام.
      </p>
      <div
        style={{
      ...
      >
        <b>الهيرو (الفيديو)</b>
        <button
          style={cfg.heroHidden ? A.btnGhost : A.btn}
          onClick={() => setCfg({ ...cfg, heroHidden: !cfg.heroHidden })}
        >
          {cfg.heroHidden ? "مخفي — إظهار" : "ظاهر — إخفاء"}
        </button>
      </div>
      {orderedKeys().map((key) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <button style={A.btnGhost} onClick={() => move(key, -1)}>
                ▲
              </button>
              <button style={A.btnGhost} onClick={() => move(key, 1)}>
                ▼
              ...
              <button
                style={hidden ? A.btnGhost : A.btn}
                onClick={() => toggle(key)}
              >
                {hidden ? "مخفي — إظهار" : "ظاهر — إخفاء"}
              </button>
            </div>
          </div>
        );
      ...
      <button
        ...
        onClick={save}
      >
        {saving ? "جارٍ الحفظ…" : "حفظ ترتيب الأقسام"}
      </button>
    </div>
  );
}
...
const HOME_TEXT_FIELDS: Record<
  string,
  { path: string; label: string; multi?: boolean }[]
> = {
  intro: [
...
export function HomeTextsEditor({ api }: { api: Api }) {
  ...
  useEffect(() => {
    api("/api/content").then((rows: any[]) => {
      ...
      if (row?.valueAr) {
        try {
          setTexts(JSON.parse(row.valueAr) || {});
        } catch {}
      }
      setLoaded(true);
    });
  ...
  const save = async () => {
    ...
    await api("/api/admin/content/home.texts", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        valueAr: JSON.stringify(clean),
        type: "json",
        section: "home",
      }),
    });
  ...
  return (
    <div style={{ ...A.card, marginTop: 16 }}>
      ...
      <p style={{ color: "#888", fontSize: 13 }}>
        اترك الحقل فارغاً ليبقى النص الافتراضي. ما تكتبه هنا يتجاوز النص
        الافتراضي على الموقع العام (لكل لغة على حدة).
      </p>
      {Object.entries(HOME_TEXT_FIELDS).map(([secKey, fields]) => {
        const label =
          HOME_SECTIONS.find((s) => s.key === secKey)?.label || secKey;
        const isOpen = open === secKey;
        const filled = fields.filter(
          (f) =>
            (texts[f.path]?.ar || "").trim() ||
            (texts[f.path]?.en || "").trim(),
        ).length;
        return (
          <div
            key={secKey}
          ...
          >
            ...
            >
              ...
              <span style={{ color: "#888", fontSize: 12 }}>
                {filled ? `${filled} حقل معدّل` : "افتراضي"}{" "}
                {isOpen ? "▲" : "▼"}
              </span>
            </button>
            {isOpen && (
                      ...
                      <div>
                        ...
                        {f.multi ? (
                          <textarea
                            style={{ ...A.input, minHeight: 70 }}
                            dir="rtl"
                            value={texts[f.path]?.ar || ""}
                            onChange={(e) =>
                              setField(f.path, "ar", e.target.value)
                            }
                          />
                        ) : (
                          <input
                      ...
                      <div>
                        ...
                        {f.multi ? (
                          <textarea
                            style={{ ...A.input, minHeight: 70 }}
                            dir="ltr"
                            value={texts[f.path]?.en || ""}
                            onChange={(e) =>
                              setField(f.path, "en", e.target.value)
                            }
                          />
                        ) : (
                          <input
