...
const t = {
  ar: {
    ...
    emergencySentNote:
      "وصلت الحالة لفريق العيادة. إذا تدهورت الحالة اتصل الآن.",
    emergencyUnavailable: "المسار الطارئ غير متاح حالياً — تواصل مباشرة",
    emergencyChannelMissing:
      "قناة الاستلام غير مهيأة — استخدم الاتصال أو واتساب",
  ...
  en: {
    ...
    consent:
      "I consent to processing my personal data for booking per the privacy policy",
    ...
    emergencyNote:
      "Do not wait for booking — call now or send the case to the team",
    ...
    emergencySentNote:
      "The clinic team received the case. If it gets worse, call now.",
    emergencyUnavailable:
      "Emergency path is currently unavailable — contact us directly",
    emergencyChannelMissing:
      "Receiving channel is not configured — use call or WhatsApp",
      ...
      <div className="flex gap-3 justify-center">
        {fallback.whatsapp && (
          <a
            ...
            href={`https://wa.me/${fallback.whatsapp}`}
            target="_blank"
            rel="noreferrer"
          >
            {L.whatsapp}
...
function normalizeSlot(
  ...
  if (typeof slot === "string") {
    if (/^\d{2}:\d{2}/.test(slot))
      return {
        iso: `${day}T${slot.slice(0, 5)}:00${RIYADH_OFFSET}`,
...
export default function HubWidget() {
  ...
  const [emergencyConfig, setEmergencyConfig] =
    useState<EmergencyConfig | null>(null);
  ...
  useEffect(() => {
    ...
    setDoctors(null);
    fetch(
      `/api/hub/${brand}/clinics/${encodeURIComponent(clinic.id)}/doctors?serviceId=${service.id}`,
    )
      .then(async (r) => {
        const j = await r.json();
        if (!r.ok) throw new Error(j.error);
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/hub/${brand}/clinics/${encodeURIComponent(clinic.id)}/availability?${q}`,
    )
      .then(async (r) => {
        const j = await r.json();
        if (!r.ok) throw new Error(j.error);
        setSlots(Array.isArray(j.availability) ? j.availability : []);
        track("view_availability", {
          brand,
          clinic: clinic.id,
          day,
  ...
  const normalizedSlots = useMemo(
    () =>
      (slots || []).map((s) => normalizeSlot(s, day)).filter(Boolean) as {
        iso: string;
        label: string;
      }[],
    [slots, day],
  );
  const morning = normalizedSlots.filter(
    (s) => Number(s.label.slice(0, 2)) < 13,
  );
  const evening = normalizedSlots.filter(
    (s) => Number(s.label.slice(0, 2)) >= 13,
  );
  ...
  async function submitDetails() {
    if (
      ...
      !form.birthdate ||
      !form.neutered ||
      !/^(\+?9665\d{8}|05\d{8})$/.test(form.phone.trim().replace(/\s/g, ""))
    ) {
  ...
  async function verifyOtpAndConfirm() {
    ...
    try {
      const v = await fetch(
        `/api/hub/${brand}/bookings/${bookingId}/otp/verify`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function choosePayment(choice: "now" | "clinic") {
    ...
    try {
      const r = await fetch(
        `/api/hub/${brand}/bookings/${bookingId}/payment-choice`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    const poll = setInterval(async () => {
      ...
      try {
        ...
        if (j.booking?.status === "confirmed") {
          ...
          setRefCode(j.ref);
          setStep("success");
          const returnAttribution = j.booking?.district
            ? { district: j.booking.district }
            : analyticsAttribution;
          ...
          if (j.payment?.status === "paid") {
            ...
            if (!alreadyTracked) {
              track("purchase", {
                brand,
                currency: j.payment.currency || "SAR",
                value: (j.payment.amount_halalas || 0) / 100,
                transaction_id: bid,
                ...returnAttribution,
              });
            }
  ...
  async function submitEmergency() {
    if (
      !emergencyForm.species ||
      !emergencyForm.symptoms.trim() ||
      !emergencyForm.eta
    ) {
      setError("required");
      return;
    ...
    try {
      ...
      if (!r.ok) {
        ...
        setError(
          j.error === "emergency_channel_unconfigured"
            ? "emergencyChannelMissing"
            : j.error === "emergency_unavailable"
              ? "emergencyUnavailable"
              : j.error || "emergencyUnavailable",
        );
        return;
      }
      setStep("emergencySent");
      track("emergency_submitted", {
        brand,
  ...
  function icsUrl(): string {
    ...
    const end = new Date(
      start.getTime() + (service.duration_minutes || 30) * 60000,
    );
    const fmt = (d: Date) =>
    ...
    const ics = [
      ...
      `DTSTART:${fmt(start)}`,
      `DTEND:${fmt(end)}`,
      `SUMMARY:${lang === "ar" ? clinic.name_ar : clinic.name_en} — ${lang === "ar" ? service.name_ar : service.name_en}`,
      "END:VEVENT",
      "END:VCALENDAR",
    ].join("\r\n");
  ...
  if (step === "emergency") {
    return (
      <div dir={dir} className="mx-auto max-w-xl p-4 space-y-4">
        ...
        <FallbackCta
          fallback={emergencyConfig?.fallback || fallback}
          lang={lang}
        />
        {emergencyConfig?.contact_only ? (
          <p className="rounded-xl border border-amber-400 p-4 text-center">
            {L.emergencyChannelMissing}
          </p>
        ) : (
          <>
            <div className="space-y-2">
              ...
              <div
                ...
                role="radiogroup"
                aria-label={L.species}
              >
                {L.speciesOptions.map((s) => (
                  <button
                    key={s}
                    onClick={() =>
                      setEmergencyForm({ ...emergencyForm, species: s })
                    }
                    className={`rounded-full border px-4 py-2 min-h-[48px] ${emergencyForm.species === s ? "bg-red-700 text-white" : ""}`}
                  >
                    {s}
                  </button>
                ))}
            ...
            <textarea
              ...
              placeholder={L.emergencySymptoms}
              value={emergencyForm.symptoms}
              onChange={(e) =>
                setEmergencyForm({ ...emergencyForm, symptoms: e.target.value })
              }
              rows={4}
              maxLength={500}
            />
            <div className="space-y-2">
              ...
              <div className="grid gap-2">
                {L.etaOptions.map((eta) => (
                  <button
                    key={eta}
                    onClick={() => setEmergencyForm({ ...emergencyForm, eta })}
                    className={`rounded-xl border px-4 py-3 text-start min-h-[48px] ${emergencyForm.eta === eta ? "bg-red-700 text-white" : ""}`}
                  >
                    {eta}
                  </button>
                ))}
            ...
            <input
              type="text"
              name="website"
              value={emergencyForm.website}
              onChange={(e) =>
                setEmergencyForm({ ...emergencyForm, website: e.target.value })
              }
              className="hidden"
              tabIndex={-1}
              autoComplete="off"
            ...
            {error && (
              <p className="text-red-600 text-sm">
                {error in L ? (L as any)[error] : L.error}
              </p>
            )}
            <div className="sticky bottom-4">
              <button
                ...
                disabled={loading}
                className="w-full rounded-xl bg-red-700 px-4 py-3 text-white min-h-[48px] disabled:opacity-50"
              >
    ...
    return (
      <div dir={dir} className="mx-auto max-w-xl p-6 text-center space-y-4">
        <p className="text-lg font-semibold">
          {fatal === "unavailable" ? L.unavailable : L.error}
        </p>
        {emergencyConfig?.enabled && (
          <button
            onClick={startEmergency}
            className="w-full rounded-xl bg-red-700 px-4 py-3 text-white min-h-[48px]"
          >
            {L.emergencyCta}
          </button>
        )}
      ...
      <div dir={dir} className="mx-auto max-w-xl p-4 space-y-6">
        ...
        <div className="sticky bottom-4">
          <button
            ...
            className="w-full rounded-xl bg-primary px-4 py-3 text-primary-foreground min-h-[48px] disabled:opacity-50"
          >
            {loading ? L.loading : L.confirmBooking}
          </button>
        </div>
        <button
          onClick={() => bookingId && requestOtp(bookingId)}
          disabled={resendAfter > 0}
          className="w-full text-center text-sm underline disabled:no-underline disabled:text-muted-foreground"
        >
  ...
  if (step === "details") {
    return (
      <div dir={dir} className="mx-auto max-w-xl p-4 space-y-4">
        <button className="text-sm underline" onClick={() => setStep("browse")}>
          ← {L.back}
        </button>
        <h2 className="text-xl font-bold">{L.details}</h2>
        <input
          className="w-full rounded-xl border p-3 min-h-[48px]"
          placeholder={L.name}
          value={form.name}
        ...
        <div className="flex gap-2">
          <input
            className="flex-1 rounded-xl border p-3 min-h-[48px]"
            placeholder={L.petName}
            value={form.petName}
        ...
        >
          {L.speciesOptions.map((s) => (
            <button
              ...
              className={`rounded-full border px-4 py-2 min-h-[48px] ${form.species === s ? "bg-primary text-primary-foreground" : ""}`}
            >
              {s}
            </button>
          ))}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {L.petSexOptions.map((o) => (
            <button
              key={o.v}
              onClick={() => setForm({ ...form, sex: o.v })}
              className={`rounded-full border px-4 py-2 min-h-[48px] ${form.sex === o.v ? "bg-primary text-primary-foreground" : ""}`}
            >
              {o.t}
            </button>
          ))}
        ...
        <label className="block text-sm text-muted-foreground">
          ...
          <input
            ...
            className="mt-1 w-full rounded-xl border p-3 min-h-[48px] text-foreground"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {L.petNeuteredOptions.map((o) => (
            <button
              key={o.v}
              onClick={() => setForm({ ...form, neutered: o.v })}
              className={`rounded-full border px-4 py-2 min-h-[48px] ${form.neutered === o.v ? "bg-primary text-primary-foreground" : ""}`}
            >
              {o.t}
            </button>
          ))}
      ...
      <div dir={dir} className="mx-auto max-w-xl p-4 space-y-4">
        ...
        {paymentWaiting ? (
          ...
          <div className="grid gap-3">
            <button
              onClick={() => choosePayment("now")}
              disabled={loading}
              className="rounded-xl border p-5 text-start hover:bg-accent min-h-[48px] disabled:opacity-50"
            >
              <span className="font-semibold block">{L.payNow}</span>
            </button>
            {paymentMode === "optional" && (
              <button
                onClick={() => choosePayment("clinic")}
                disabled={loading}
                className="rounded-xl border p-5 text-start hover:bg-accent min-h-[48px] disabled:opacity-50"
              >
  ...
  return (
    <div dir={dir} className="mx-auto max-w-xl p-4 space-y-6">
      ...
      {emergencyConfig?.enabled && (
        <button
          onClick={startEmergency}
          className="w-full rounded-xl border border-red-300 bg-red-50 px-4 py-3 font-semibold text-red-700 min-h-[48px]"
        >
          {L.emergencyCta}
        </button>
      )}
            ...
            <div className="grid gap-3">
              {clinics.map((c) => (
                <button
                  ...
                  onClick={() => {
                    setClinic(c);
                    setPaymentMode(c.payment_mode || brandPaymentMode);
                  }}
                  className="rounded-xl border p-4 text-start hover:bg-accent transition min-h-[48px]"
                >
      ...
      {clinic && !service && (
        <section className="space-y-3">
          <button
            ...
            onClick={() => {
              setClinic(null);
              setPaymentMode(brandPaymentMode);
            }}
          >
            ← {L.back}
          </button>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`rounded-lg border px-3 py-2 min-h-[48px] ${!doctor ? "bg-primary text-primary-foreground" : ""}`}
              >
                {L.anyDoctor}
              </button>
              {(doctors || []).map((d) => (
                <button
                  key={d.id}
                  onClick={() => setDoctor(d)}
                  className={`rounded-lg border px-3 py-2 min-h-[48px] ${doctor?.id === d.id ? "bg-primary text-primary-foreground" : ""}`}
                >
                  {d.name}
                </button>
              ))}
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="rounded-lg border p-2 min-h-[48px]"
            />
          </div>
          <div>
            {loading || slots === null ? (
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {group.map((s) => (
                          <button
                            ...
                            className="rounded-lg border p-2 text-center min-h-[48px] hover:bg-accent"
                          >
                            {s.label}
                          </button>
                        ))}
