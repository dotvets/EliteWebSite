...
export default function TestimonialsSection() {
  ...
  return (
    <div className="max-w-7xl mx-auto overflow-x-hidden">
      <SectionHeader
        title={language === "ar" ? "ماذا يقول عملاؤنا" : "What Our Clients Say"}
        titleTestId="text-testimonials-title"
      />
      <AnimatedContent
        variant="staggerGrid"
      ...
      >
        {items.map((t) => {
          const rating = Math.max(
            0,
            Math.min(5, parseInt(t.rating || "5", 10) || 5),
          );
          return (
            <Card
              key={t.id}
            ...
            >
              ...
              <p className="text-foreground text-sm leading-relaxed mb-4">
                “{pick(t.textAr, t.textEn)}”
              </p>
              <div className="flex items-center gap-3">
                {t.image ? (
                  <img
                    src={t.image}
                    alt={pick(t.customerAr, t.customerEn)}
                ...
                <div>
                  <div className="font-semibold text-sm">
                    {pick(t.customerAr, t.customerEn)}
                  </div>
                  {t.branch && (
