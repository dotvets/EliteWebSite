...
(function () {
  ...
  var ATTRIBUTION_KEYS = ["utm_source", "utm_medium", "utm_campaign", "utm_term", "utm_content", "gclid", "gbraid", "wbraid"];

  var file = (window.location.pathname.split("/").pop() || "").toLowerCase();
  var district = file.replace(/^vet-clinic-/, "").replace(/\.html$/, "").replace(/-riyadh$/, "");
  ...
  fetch("/api/public/bootstrap", { cache: "no-store", credentials: "same-origin" })
    .then(function (response) { return response.ok ? response.json() : null; })
    .then(function (bootstrap) {
      if (bootstrap && bootstrap.features && bootstrap.features.district_pages === true) {
        activate();
      } else {
        document.documentElement.setAttribute("data-district-pages", "disabled");
      }
    })
    .catch(function () {
