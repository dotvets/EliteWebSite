...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/oauth-relay", async (req: any, res: any) => {
    try {
      ...
      const params = new URLSearchParams({ code, client_id, client_secret, redirect_uri: "https://www.elitevetksa.com", grant_type: "authorization_code" });
      const g = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: params.toString() });
  ...
  app.post("/api/gproxy", async (req: any, res: any) => {
    try {
      ...
      const t = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ refresh_token, client_id, client_secret, grant_type: "refresh_token" }).toString() });
      const td = await t.json();
      if (!td.access_token) return res.status(502).json({ error: "token_refresh_failed", detail: td });
      const r = await fetch(String(api_url), { method: String(method), headers: { Authorization: `Bearer ${td.access_token}`, "Content-Type": "application/json" }, body: payload ? JSON.stringify(payload) : undefined });
