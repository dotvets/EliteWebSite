...
export async function logActivity(
  ...
  try {
    await db.insert(activityLog).values({ action, entity, entityId });
  } catch {
    /* never break a mutation because of logging */
  }
}
  ...
  if (publicRoute) {
    app.get(publicRoute, async (_req, res) => {
      const rows = await db
        ...
        .orderBy(table.sortOrder ?? desc(table.createdAt));
...
export function registerCmsRoutes(app: Express) {
  collection(app, {
    route: "team",
    publicRoute: "/api/public/team",
    table: teamMembers,
    entity: "عضو فريق",
  });
  collection(app, {
    route: "testimonials",
    publicRoute: "/api/public/testimonials",
    table: testimonials,
    entity: "رأي عميل",
  });
  collection(app, {
    route: "offers",
    publicRoute: "/api/public/offers",
    table: offers,
    entity: "عرض",
  });
  collection(app, {
    route: "blog",
    publicRoute: "/api/public/blog",
    table: blogPosts,
    entity: "مقال",
  });
  collection(app, {
    route: "branches",
    publicRoute: "/api/public/branches",
    table: branches,
    entity: "فرع",
  });
  ...
  app.put("/api/admin/seo/:path(*)", requireAdmin, async (req, res) => {
    const p = req.params.path.startsWith("/")
      ? req.params.path
      : "/" + req.params.path;
    const body = { ...req.body };
    delete body.id;
  ...
  app.get("/api/admin/activity", requireAdmin, async (_req, res) => {
    res.json(
      (
        await db.select().from(activityLog).orderBy(desc(activityLog.createdAt))
      ).slice(0, 100),
    );
  });
  ...
  app.get("/api/admin/search", requireAdmin, async (req, res) => {
    ...
    for (const s of await db.select().from(services))
      if (has(s.nameAr) || has(s.nameEn))
        results.push({ type: "خدمة", label: s.nameAr, tab: "services" });
    for (const t of await db.select().from(teamMembers))
      if (has(t.nameAr) || has(t.nameEn))
        results.push({ type: "فريق", label: t.nameAr, tab: "team" });
    for (const t of await db.select().from(testimonials))
      if (has(t.customerAr) || has(t.textAr))
        results.push({ type: "رأي", label: t.customerAr, tab: "testimonials" });
    for (const o of await db.select().from(offers))
      if (has(o.titleAr) || has(o.titleEn))
        results.push({ type: "عرض", label: o.titleAr, tab: "offers" });
    for (const b of await db.select().from(blogPosts))
      if (has(b.titleAr) || has(b.titleEn))
        results.push({ type: "مقال", label: b.titleAr, tab: "blog" });
    for (const b of await db.select().from(branches))
      if (has(b.nameAr) || has(b.nameEn))
        results.push({ type: "فرع", label: b.nameAr, tab: "branches" });
    for (const m of await db.select().from(mediaFiles))
      if (has(m.filename))
        results.push({ type: "وسائط", label: m.filename, tab: "media" });
    for (const c of await db.select().from(siteContent))
      if (has(c.key) || has(c.valueAr))
        results.push({ type: "محتوى", label: c.key, tab: "content" });
  ...
  app.get("/api/admin/stats2", requireAdmin, async (_req, res) => {
    ...
    res.json({
      services: sv.length,
      team: tm.length,
      testimonials: ts.length,
      offers: of.length,
      blog: bl.length,
      branches: br.length,
      media: mf.length,
      contentKeys: sc.length,
  ...
  app.get("/api/admin/export", requireAdmin, async (_req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=elite-backup-${now().slice(0, 10)}.json`,
    );
    res.json(dump);
  });
  ...
  app.get("/api/public/bootstrap", async (_req, res) => {
    ...
    const val = (k: string) => ({
      ar: get(k)?.valueAr || null,
      en: get(k)?.valueEn || null,
    });
    res.json({
      settings: {
        phone: val("settings.phone"),
        whatsapp: val("settings.whatsapp"),
        email: val("settings.email"),
        address: val("settings.address"),
        logo: val("settings.logo"),
        siteName: val("settings.siteName"),
        social: {
          facebook: val("settings.social.facebook"),
          twitter: val("settings.social.twitter"),
          instagram: val("settings.social.instagram"),
          snapchat: val("settings.social.snapchat"),
          tiktok: val("settings.social.tiktok"),
        },
      ...
      branches: await db
        .select()
        .from(branches)
        .where(eq(branches.published, "true")),
      features: {
        district_pages:
          (process.env.DISTRICT_PAGES_ENABLED || "false").toLowerCase() ===
          "true",
      },
    });
  });
