...
export function externallyConfigured(provider: ProviderSpec): boolean {
  return provider.secretRefs
    .filter((r) => r.required)
    .every((r) => !!process.env[r.envName]);
}
...
export async function logEvent(
  ...
  await pool
    ...
    .catch((e) =>
      console.error("[integrations] audit write failed:", e?.message),
    );
}
...
async function getHealthRow(providerKey: string, scopeId: string | null) {
  ...
  if (failures >= 5) {
    const openedAt = h.circuit_opened_at
      ? new Date(h.circuit_opened_at).getTime()
      : 0;
...
function computeStatus(
  ...
  row: ConfigRow | null,
  health: Awaited<ReturnType<typeof getHealthRow>>,
): IntegrationStatus {
  ...
  if (
    health &&
    (health.consecutiveFailures > 0 || health.circuitState !== "closed")
  )
...
export async function getIntegrationView(
  ...
  if (row && !visibleScope(row.scope_type, row.scope_id))
    return hiddenView(provider);
...
function hiddenView(provider: ProviderSpec): IntegrationView {
  return {
    ...
    environment: null,
    enabled: false,
    status: externallyConfigured(provider)
...
export async function upsertConfig(opts: {
  ...
  if (!provider.allowedScopes.includes(opts.scopeType))
    return { ok: false, error: "invalid_scope" };
  if (!provider.allowedEnvironments.includes(opts.environment))
    return { ok: false, error: "invalid_environment" };
  if (opts.scopeType === "brand" && isProtectedBrand(opts.scopeId))
    return { ok: false, error: "protected_asset" };
  if (!/^[a-zA-Z0-9*:_-]{1,64}$/.test(opts.scopeId))
    return { ok: false, error: "invalid_scope_id" };
  ...
  const refs = Object.fromEntries(
    provider.secretRefs.map((r) => [
      r.key,
      { kind: "render_env", name: r.envName },
    ]),
  );
  ...
  if (existing) {
    ...
    await logEvent(
      ...
      "config.update",
      {
        fields: Object.keys(v.clean),
        previous_scope: `${existing.scope_type}:${existing.scope_id}`,
      },
      "ok",
    );
  } else {
...
export async function setEnabled(
  ...
  if (!visibleScope(row.scope_type, row.scope_id))
    return { ok: false, error: "protected_asset" };
  await pool.query(
    `UPDATE hub_integrations_configs SET enabled = $2, updated_by = $3, updated_at = NOW() WHERE id = $1`,
    [row.id, enabled, actor],
  );
  await logEvent(
    actor,
    providerKey,
...
export async function listRoutes(): Promise<any[]> {
  const r = await pool.query(
    `SELECT * FROM hub_messaging_routes ORDER BY brand_id, purpose`,
  );
...
export async function setRoute(opts: {
  ...
  if (isProtectedBrand(opts.brandId))
    return { ok: false, error: "protected_asset" };
  const provider = getProvider(opts.providerKey);
  if (!provider || provider.category !== "messaging")
    return { ok: false, error: "invalid_provider" };
  if (!ROUTING_PURPOSES.includes(opts.purpose))
  ...
  if (
    ...
    (view.status !== "ready" &&
      ...
      view.status !== "configured_externally")
  ) {
