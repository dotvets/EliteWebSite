...
function validateRow(
  row: any,
  _idx: number,
): { ok: boolean; errors: string[]; data?: any } {
  const errors: string[] = [];
  const nameAr = String(row.nameAr ?? row["Service Name AR"] ?? "").trim();
  const nameEn = String(row.nameEn ?? row["Service Name EN"] ?? "").trim();
  const descriptionAr =
    String(row.descriptionAr ?? row["Description AR"] ?? "").trim() || null;
  const descriptionEn =
    String(row.descriptionEn ?? row["Description EN"] ?? "").trim() || null;
  const priceRaw = String(row.price ?? row["Price"] ?? "").trim();
  const currency = (
    String(row.currency ?? row["Currency"] ?? "SAR").trim() || "SAR"
  ).toUpperCase();
  const activeRaw = String(row.isActive ?? row["Active"] ?? "yes")
    .trim()
    .toLowerCase();
  if (!nameAr) errors.push("اسم الخدمة بالعربية مفقود");
  if (!nameEn) errors.push("Service Name EN missing");
  if (!PRICE_RE.test(priceRaw))
    errors.push("السعر غير صالح — يجب أن يكون رقماً");
  const isActive = ["no", "0", "false", "inactive", "غير مفعلة", "لا"].includes(
    activeRaw,
  )
    ? "false"
    : "true";
...
export function registerServiceRoutes(app: Express) {
  ...
  app.patch("/api/admin/services/:id", requireAdmin, async (req, res) => {
    ...
    await db
      .update(services)
      .set({ ...v.data, updatedAt: new Date().toISOString() })
      .where(eq(services.id, req.params.id));
    res.json({ ok: true });
  });
  ...
  app.post("/api/admin/services/import", requireAdmin, async (req, res) => {
    ...
    rows.forEach((row: any, i: number) => {
      ...
      if (!v.ok) {
        results.push({ row: i + 1, ok: false, errors: v.errors });
        return;
      }
      ...
      if (seen.has(key)) {
        results.push({
          ...
          ok: false,
          skipped: true,
          errors: ["مكررة — الخدمة موجودة مسبقاً"],
        });
        return;
      }
      seen.add(key);
  ...
  app.get("/api/admin/services/template", requireAdmin, (_req, res) => {
    const csv =
      "Service Name AR,Service Name EN,Description AR,Description EN,Price,Currency,Active\n" +
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=services-template.csv",
    );
    res.send("﻿" + csv);
  });
}
