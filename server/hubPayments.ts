...
function verifyWebhookSignature(
  ...
  const fields = (
    process.env.MYFATOORAH_WEBHOOK_SIGNATURE_FIELDS || "InvoiceId,InvoiceStatus"
  )
    .split(",")
    .map((f) => f.trim())
    .filter(Boolean);
...
export function registerHubPaymentRoutes(app: Express) {
  ...
  app.post("/api/hub/:brand/bookings/:id/payment-choice", async (req, res) => {
    try {
      ...
      if (!["held", "pending_payment"].includes(b.status))
        return res.status(409).json({ error: `invalid_state_${b.status}` });

      const brand = (
        await pool.query(`SELECT * FROM hub_brands WHERE brand = $1`, [b.brand])
      ).rows[0];
      const clinic = (
        await pool.query(`SELECT * FROM hub_clinics WHERE id = $1`, [
          b.clinic_id,
        ])
      ).rows[0];
      ...
      if (mode === "off")
        return res.status(400).json({ error: "payment_disabled" });
      ...
      if (mode === "optional" && choice === "clinic") {
        ...
        if (result.outcome === "confirmed")
          return res.json({ confirmed: true, ref: result.ref });
      ...
      if (!amountHalalas || amountHalalas <= 0)
        return res.status(500).json({ error: "payment_amount_unconfigured" });
      if (!mfConfigured())
        return res.status(503).json({ error: "payment_provider_unconfigured" });

      const idem = `pay:${b.id}`;
      const existing = await pool.query(
        `SELECT * FROM hub_payments WHERE idempotency_key = $1 LIMIT 1`,
        [idem],
      );
      ...
      if (!invoice?.IsSuccess) {
        await audit("system", "payment.initiate_failed", "hub_bookings", b.id, {
          reason: String(invoice?.Message || "unknown").slice(0, 200),
        });
      ...
      await pool.query(
        ...
        [
          paymentId,
          b.id,
          String(invoice.Data.InvoiceId),
          amountHalalas,
          idem,
          JSON.stringify({ invoice_url: invoice.Data.InvoiceURL }),
        ],
      );
      await pool.query(
        `UPDATE hub_bookings SET status = 'pending_payment', payment_id = $2, updated_at = NOW() WHERE id = $1`,
        [b.id, paymentId],
      );
      await enqueueNotification({
        ...
        payload: {
          body:
            b.locale === "ar"
              ? `أكمل الدفع لتأكيد حجزك: ${invoice.Data.InvoiceURL}`
              : `Complete payment to confirm your booking: ${invoice.Data.InvoiceURL}`,
        },
      ...
      await audit(
        ...
        "hub_payments",
        paymentId,
        { booking: b.id, invoice: String(invoice.Data.InvoiceId) },
      );
  ...
  app.post(
    "/api/hub/payments/myfatoorah/webhook",
    async (req: any, res: Response) => {
      // Always answer 200 immediately; process async (Part 7.2).
      res.status(200).json({ received: true });
      const raw = req.rawBody;
      const body = req.body || {};
      const signature = req.headers["myfatoorah-signature"] as
        string | undefined;
      setImmediate(async () => {
        try {
          if (!process.env.MYFATOORAH_WEBHOOK_SECRET) {
            await audit(
              "system",
              "payment.webhook_blocked_no_secret",
              "hub_payments",
              "unknown",
              { note: "MYFATOORAH_WEBHOOK_SECRET missing — go-live blocker" },
            );
            return;
          }
          ...
          if (!verifyWebhookSignature(raw, data, signature)) {
            await audit(
              "system",
              "payment.webhook_bad_signature",
              "hub_payments",
              String(data?.InvoiceId || "unknown"),
              {},
            );
            return;
          }
          ...
          const bkBrand = await pool
            .query(
              `SELECT b.brand FROM hub_payments hp JOIN hub_bookings b ON b.id = hp.booking_id WHERE hp.provider_invoice_id = $1 LIMIT 1`,
              [invoiceId],
            )
            .catch(() => ({ rows: [] as any[] }));
          ...
          const pay = await pool.query(
            `SELECT * FROM hub_payments WHERE provider_invoice_id = $1 LIMIT 1`,
            [invoiceId],
          );
          if (!pay.rows[0]) {
            await audit(
              "system",
              "payment.webhook_unknown_invoice",
              "hub_payments",
              invoiceId,
              {},
            );
          ...
          await pool.query(
            `UPDATE hub_payments SET raw_webhook_json = raw_webhook_json || $2::jsonb WHERE id = $1`,
            [p.id, JSON.stringify({ last_webhook: body })],
          );
          ...
          if (status === "Paid" && p.status !== "paid") {
            await pool.query(
              `UPDATE hub_payments SET status = 'paid', updated_at = NOW() WHERE id = $1 AND status != 'paid'`,
              [p.id],
            );
            await audit("system", "payment.paid", "hub_payments", p.id, {
              booking: booking.id,
            });
            ...
            if (result.outcome !== "confirmed") {
              await audit(
                "system",
                "payment.confirm_after_paid_failed",
                "hub_bookings",
                booking.id,
              ...
              if (receiptUrl) {
                await enqueueNotification({
                  ...
                  payload: {
                    body:
                      booking.locale === "ar"
                        ? `إيصال الدفع لحجزك: ${receiptUrl}`
                        : `Payment receipt for your booking: ${receiptUrl}`,
                  },
                  scheduledAt: new Date(),
                  respectQuietHours: false,
                });
            ...
            await pool.query(
              `UPDATE hub_payments SET status = 'failed', updated_at = NOW() WHERE id = $1`,
              [p.id],
            );
            // Release hold + one-tap re-pay/fresh-booking notification (Part 7.1)
            await pool.query(
              `UPDATE hub_bookings SET status = 'failed', updated_at = NOW() WHERE id = $1 AND status = 'pending_payment'`,
              [booking.id],
            );
            ...
            await enqueueNotification({
              ...
              payload: {
                body:
                  booking.locale === "ar"
                    ? `لم يتم الدفع — يمكنك بدء حجز جديد من هنا: ${freshBookingUrl}`
                    : `Payment failed — start a fresh booking here: ${freshBookingUrl}`,
              },
              scheduledAt: new Date(),
              respectQuietHours: false,
            });
            await audit("system", "payment.failed", "hub_payments", p.id, {
              booking: booking.id,
          ...
          console.error(
            JSON.stringify({
              level: "error",
              msg: "payment_webhook_processing_failed",
  ...
  app.get(
    "/api/hub/admin/payments/reconciliation",
    requireAdmin,
    async (_req, res) => {
      try {
        ...
        const discrepancies = rows.rows.filter(
          (r) =>
            (r.status === "paid" && r.booking_status !== "confirmed") ||
            (r.status === "initiated" &&
              Date.now() - new Date(r.updated_at).getTime() > 24 * 3600_000),
        );
        res.json({
          lastRun: reconLastRun?.toISOString() || null,
          payments: rows.rows,
          discrepancies,
        });
      } catch (error: any) {
...
async function maybeRunReconciliation() {
  ...
  if (!(
    riyadh.getHours() === 3 &&
    riyadh.getMinutes() >= 30 &&
    riyadh.getMinutes() < 32
  ))
    return;
  if (reconLastRun && reconLastRun.toISOString().slice(0, 10) === key) return;
  const lock = await pool
    .query(`SELECT pg_try_advisory_lock($1) AS got`, [RECON_ADVISORY_LOCK])
    .catch(() => ({ rows: [{ got: false }] }));
  if (!lock.rows[0]?.got) return;
  try {
    let checkedPayments = 0;
    const initiated = await pool.query(
      `SELECT * FROM hub_payments WHERE status = 'initiated' AND updated_at < NOW() - interval '15 minutes'`,
    );
    for (const p of initiated.rows) {
      checkedPayments += 1;
      try {
        const st = await mfPost("/v2/getPaymentStatus", {
          Key: p.provider_invoice_id,
          KeyType: "InvoiceId",
        });
        const upstream = st?.Data?.InvoiceStatus;
        if (upstream && upstream !== "Pending" && upstream !== "paid") {
          await audit("system", "recon.discrepancy", "hub_payments", p.id, {
            type: "stale_initiated_payment",
            local: p.status,
            upstream,
          });
        }
      } catch (e: any) {
        await audit("system", "recon.check_failed", "hub_payments", p.id, {
    ...
    for (const row of paidMismatches.rows) {
      await audit("system", "recon.discrepancy", "hub_payments", row.id, {
    ...
    for (const row of confirmed.rows) {
      ...
      try {
        await digitailRequest(
          "GET",
          `/appointments/${encodeURIComponent(row.digitail_appointment_id)}`,
        );
      } catch (e: any) {
        if (e?.status === 404) {
          await audit("system", "recon.discrepancy", "hub_bookings", row.id, {
            type: "missing_upstream_appointment",
            appointment: row.digitail_appointment_id,
          });
        } else {
          await audit("system", "recon.check_failed", "hub_bookings", row.id, {
            status: e?.status || 0,
          });
        }
      }
    ...
    await audit("system", "recon.completed", "hub_payments", "nightly", {
      checkedPayments,
      checkedAppointments,
      paidMismatches: paidMismatches.rows.length,
    });
    reconLastRun = new Date();
  } finally {
    await pool
      .query(`SELECT pg_advisory_unlock($1)`, [RECON_ADVISORY_LOCK])
      .catch(() => {});
  }
}
