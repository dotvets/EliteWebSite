...
function hashOtp(phone: string, code: string): string {
  return crypto
    .createHash("sha256")
    .update(`otp:${phone}:${code}:${process.env.SESSION_SECRET || ""}`)
    .digest("hex");
}

export function signCustomerCookie(customerId: string): string {
  const sig = crypto
    .createHmac("sha256", process.env.SESSION_SECRET || "hub")
    .update(customerId)
    .digest("base64url");
...
function safeErr(res: Response, error: any, code: string, status = 500) {
  ...
  res
    .status(Number(error?.status) && error.status < 500 ? error.status : status)
    .json({ error: code });
}
...
async function fetchDaySlots(
  ...
  if (serviceId && serviceId !== "0")
    params.set("filter[visit_type_id]", serviceId);
  else params.set("filter[duration]", String(duration));
  const data = await digitailRequest(
    "GET",
...
function slotMatches(slots: any[], startIso: string): boolean {
  ...
  for (const s of flatten(slots)) {
    ...
    if (/^\d{2}:\d{2}/.test(s)) {
      const riyadhTarget = new Date(target)
...
// ---------------------------------------------------------------------------
async function findOrCreatePetParent(
  clinicDigitailId: number,
  name: string,
  phone: string,
): Promise<string> {
  try {
    const found = await digitailRequest(
      "GET",
      `/pet-parents?filter%5Bphone%5D=${encodeURIComponent(phone)}`,
    );
...
async function resolveBreedId(
  speciesId: number,
  breedName?: string,
): Promise<number> {
  const { value } = await cached(
    `digitail:breeds:${speciesId}`,
    6 * 3600,
    async () => {
      const data = await digitailRequest(
        "GET",
        `/breeds?filter%5Bspecies_id%5D=${speciesId}`,
      );
      return data?.data || [];
    },
  );
  ...
  const labelOf = (b: any) =>
    String(b?.breed || b?.label || b?.name || "").toLowerCase();
  if (breedName) {
    const exact = list.find(
      (b: any) => labelOf(b) === breedName.trim().toLowerCase(),
    );
  ...
  throw Object.assign(new Error("no_breed_available_upstream"), {
    status: 502,
  });
}
...
async function createDigitailPet(
  clinicDigitailId: number,
  vetId: string,
  parentId: string,
  pet: {
  ...
  try {
    ...
    if (e?.status && e.status >= 400 && e.status < 500) {
      ...
      const resp = await fetch(`${cfg.apiBaseUrl}/pets`, {
        method: "POST",
        headers: {
...
function renderTemplate(
  ...
  const defaults: Record<string, string> = {
    otp:
      locale === "ar"
        ? `رمز التحقق الخاص بك: ${vars.code} — صالح 10 دقائق`
        : `Your verification code: ${vars.code} (valid 10 minutes)`,
    confirm:
      locale === "ar"
    ...
    remind_24h:
      locale === "ar"
        ? `تذكير: موعدك ${vars.ref} غداً الساعة ${vars.time} في ${vars.clinic}`
        : `Reminder: your appointment ${vars.ref} is tomorrow at ${vars.time} at ${vars.clinic}`,
    remind_2h:
      locale === "ar"
        ? `تذكير: موعدك ${vars.ref} بعد ساعتين في ${vars.clinic}`
        : `Reminder: your appointment ${vars.ref} is in 2 hours at ${vars.clinic}`,
  };
...
export function registerHubBookingRoutes(app: Express) {
  ...
  app.post("/api/hub/:brand/bookings", async (req, res) => {
    ...
    try {
      ...
      if (today.rows[0].c >= phoneDailyCap())
        return res.status(429).json({ error: "daily_booking_cap_reached" });
      ...
      const day = start
        .toLocaleString("sv-SE", { timeZone: clinic.timezone || "Asia/Riyadh" })
        .slice(0, 10);
      ...
      const existing = await pool.query(
        `SELECT * FROM hub_bookings WHERE idempotency_key = $1 LIMIT 1`,
      ...
      if (existing.rows[0]) {
        ...
        return res.json({
          ...
          ref: referenceCode(brand.brand, b.id),
          idempotent: true,
        });
      }
      ...
      const resolvedPayment = await resolvePaymentModeForHold(
        brand,
        clinic,
        phone,
      );
      ...
      await pool.query(
        ...
        [
          ...
          input.customerName,
          phone,
          JSON.stringify(input.pet),
          JSON.stringify(serviceJson),
          new Date(Date.now() + holdMinutes * 60_000),
          input.locale,
          JSON.stringify(input.source || {}),
          resolvedPayment.mode,
        ],
      );
      await audit(`customer:anon`, "booking.create", "hub_bookings", id, {
        brand: brand.brand,
        clinic: clinic.id,
        phone: maskPhone(phone),
      });
      if (resolvedPayment.source === "dynamic_rule") {
        await audit("system", "deposit.dynamic_required", "hub_bookings", id, {
          mode: resolvedPayment.mode,
          no_show_count: resolvedPayment.no_show_count ?? 0,
        });
      }
      const created = await pool.query(
        `SELECT * FROM hub_bookings WHERE id = $1`,
        [id],
      );
      res.json({
        booking: publicBooking(created.rows[0]),
        ref: referenceCode(brand.brand, id),
      });
    } catch (error: any) {
  ...
  app.post("/api/hub/:brand/bookings/:id/otp/request", async (req, res) => {
    ...
    try {
      ...
      if (!b) return;
      if (b.customer_phone_verified)
        return res.status(400).json({ error: "already_verified" });

      const consent = req.body?.consent === true;
      const consentAt =
        typeof req.body?.consentAt === "string" ? req.body.consentAt : null;
      ...
      if (
        ...
        return res
          .status(429)
          .json({ error: "otp_resend_cooldown", retry_after_seconds: wait });
      }
      const hourly = await pool.query(
        `SELECT COUNT(*)::int AS c FROM hub_otp_codes WHERE phone = $1 AND created_at > NOW() - interval '1 hour'`,
        [b.customer_phone],
      );
      if (hourly.rows[0].c >= OTP_REQUESTS_PER_PHONE_PER_HOUR)
        return res.status(429).json({ error: "otp_hourly_cap" });

      const code = String(crypto.randomInt(0, 1_000_000)).padStart(6, "0");
      await pool.query(
        `INSERT INTO hub_otp_codes (id, phone, code_hash, expires_at) VALUES ($1,$2,$3,$4)`,
        [
          crypto.randomUUID(),
          b.customer_phone,
      ...
      await pool.query(
        `UPDATE hub_bookings SET source_json = source_json || $2::jsonb, updated_at = NOW() WHERE id = $1`,
        [b.id, JSON.stringify({ pdpl_consent_at: consentAt })],
      );
      ...
      const body = renderTemplate(
        ...
        { code },
        b.locale,
        (b.brand_config || {}).templates,
      );
      await provider.sendSms(
        b.customer_phone,
        body,
        (b.brand_config || {}).smsSenderId,
      );
      await audit(`customer:anon`, "otp.request", "hub_bookings", b.id, {
  ...
  app.post("/api/hub/:brand/bookings/:id/otp/verify", async (req, res) => {
    ...
    try {
      ...
      if (otp.attempts >= OTP_MAX_ATTEMPTS)
        return res.status(429).json({ error: "otp_attempts_exceeded" });
      if (hashOtp(b.customer_phone, code) !== otp.code_hash) {
        await pool.query(
          `UPDATE hub_otp_codes SET attempts = attempts + 1 WHERE id = $1`,
          [otp.id],
        );
        return res.status(400).json({
          error: "otp_incorrect",
      ...
      await pool.query(
        `UPDATE hub_otp_codes SET consumed_at = NOW() WHERE id = $1`,
        [otp.id],
      );
      ...
      let customer = (
        await pool.query(
          `SELECT * FROM hub_customers WHERE phone = $1 LIMIT 1`,
          [b.customer_phone],
        )
      ).rows[0];
      if (!customer) {
        ...
        await pool.query(
          `INSERT INTO hub_customers (id, phone, name, locale, pdpl_consent_at) VALUES ($1,$2,$3,$4,$5)`,
          [cid, b.customer_phone, b.customer_name, b.locale, consentAt],
        );
        customer = (
          await pool.query(`SELECT * FROM hub_customers WHERE id = $1`, [cid])
        ).rows[0];
      } else if (!customer.pdpl_consent_at && consentAt) {
        await pool.query(
          `UPDATE hub_customers SET pdpl_consent_at = $2 WHERE id = $1`,
          [customer.id, consentAt],
        );
      }
      ...
      await pool.query(
        `UPDATE hub_bookings SET customer_phone_verified = true, customer_id = $2, pet_id = $3, updated_at = NOW() WHERE id = $1`,
        [b.id, customer.id, petId],
      );
      await audit(
      ...
      res.cookie("hub_customer", signCustomerCookie(customer.id), {
        httpOnly: true,
        secure: true,
        sameSite: "lax",
  ...
  app.post("/api/hub/:brand/bookings/:id/confirm", async (req, res) => {
    ...
    try {
      const b = await loadBookingForWrite(req, res);
      if (!b) return;
      const result = await runConfirmPipeline(
        b,
        `customer:${b.customer_id || "anon"}`,
      );
      switch (result.outcome) {
        ...
        case "conflict":
          return res
            .status(409)
            .json({ error: "slot_taken", alternatives: result.alternatives });
        case "invalid_state":
          return res
            .status(409)
            .json({ error: `invalid_state_${result.state}` });
        case "otp_required":
        ...
        default:
          return res
            .status(503)
            .json({ error: "upstream_unavailable_keep_hold" });
      }
    } catch (error: any) {
  ...
  app.post("/api/hub/:brand/bookings/:id/cancel", async (req, res) => {
    ...
    try {
      ...
      if (!["held", "confirmed"].includes(b.status))
        return res.status(409).json({ error: `invalid_state_${b.status}` });

      const startMs = Date.parse((b.service_json || {}).starts_at || "");
      const moreThan2h =
        !Number.isNaN(startMs) && startMs - Date.now() > 2 * 3600_000;
      if (b.status === "confirmed" && moreThan2h) {
        ...
        if (
          !/^\d{6}$/.test(code) ||
          !otp ||
          hashOtp(b.customer_phone, code) !== otp.code_hash
        ) {
      ...
      if (b.status === "confirmed" && b.digitail_appointment_id) {
        await digitailRequest(
          "POST",
          `/appointments/${encodeURIComponent(b.digitail_appointment_id)}/cancel`,
        );
      }
      await pool.query(
        `UPDATE hub_bookings SET status = 'cancelled', updated_at = NOW() WHERE id = $1`,
        [b.id],
      );
      await audit(
        `customer:${b.customer_id || "anon"}`,
    ...
    try {
      ...
      if (b.payment_id) {
        const p = await pool.query(
          `SELECT status, amount_halalas, currency FROM hub_payments WHERE id = $1`,
          [b.payment_id],
        );
      ...
      res.json({
        booking: publicBooking(b),
        ref: referenceCode(b.brand, b.id),
        payment,
      });
    } catch (error: any) {
    ...
    try {
      ...
      res.json({
        bookings: rows.rows.map((b) => ({
          ...b,
          customer_phone: maskPhone(b.customer_phone),
          ref: referenceCode(b.brand, b.id),
        })),
      });
    } catch (error: any) {
  ...
  app.patch("/api/hub/admin/bookings/:id", requireAdmin, async (req, res) => {
    try {
      ...
      if ("payment_mode_override" in (req.body || {})) {
        ...
        if (!(
          o === null ||
          ["off", "optional", "required_deposit"].includes(String(o))
        )) {
          return res
            .status(400)
            .json({ error: "invalid_payment_mode_override" });
        }
      ...
      const r = await pool.query(
        `UPDATE hub_bookings SET ${updates.join(", ")}, updated_at = NOW() WHERE id = $${values.length} RETURNING id, brand, status, payment_mode_override`,
        values,
      );
  ...
  app.get("/api/hub/admin/notifications", requireAdmin, async (_req, res) => {
    try {
      const rows = await pool.query(
        `SELECT id, booking_id, kind, channel, to_phone, template_key, status, attempt_count, scheduled_at, sent_at FROM hub_notifications ORDER BY created_at DESC LIMIT 100`,
      );
      res.json({
        notifications: rows.rows.map((n) => ({
          ...n,
          to_phone: maskPhone(n.to_phone),
        })),
      });
    } catch (error: any) {
  ...
  app.get("/api/hub/health", requireAdmin, async (_req, res) => {
    try {
      const q = await pool.query(
        `SELECT status, COUNT(*)::int AS c FROM hub_notifications GROUP BY status`,
      );
      const counts: Record<string, number> = {};
      for (const r of q.rows) counts[r.status] = r.c;
      const recon = await pool.query(
        `SELECT created_at FROM hub_audit_log WHERE action = 'recon.completed' ORDER BY created_at DESC LIMIT 1`,
      );
      const emergencyQ = await pool.query(
        `SELECT status, COUNT(*)::int AS c FROM hub_emergency_requests GROUP BY status`,
      );
      ...
      res.json({
        ...
        messaging: {
          provider: (process.env.MESSAGING_PROVIDER || "stub").toLowerCase(),
        ...
        emergency: {
          enabled:
            (process.env.EMERGENCY_PATH_ENABLED || "false").toLowerCase() ===
            "true",
          requests: emergencyCounts,
        },
        notifications: counts,
        sweeper: {
          enabled: sweeperEnabled(),
          lastRun: sweeperLastRun?.toISOString() || null,
...
async function loadBookingForWrite(
  ...
  const r = await pool.query(
    `SELECT * FROM hub_bookings WHERE id = $1 AND brand = $2 LIMIT 1`,
    [id, String(req.params.brand || "")],
  );
  const b = r.rows[0];
  if (!b) {
  ...
  if (opts.requirePhone !== false) {
    ...
    const supplied = normalizeSaudiPhone(
      String(req.body?.phone || req.query?.phone || ""),
    );
    const cookieCustomer = verifyCustomerCookie(
      req.headers.cookie
        ?.split(/;\s*/)
        .find((c) => c.startsWith("hub_customer="))
        ?.split("=")[1],
    );
    if (
      supplied !== b.customer_phone &&
      (!cookieCustomer || cookieCustomer !== b.customer_id)
    ) {
...
function publicBooking(b: any) {
  return {
    ...
    district:
      typeof b.source_json?.district === "string" &&
      /^[a-z0-9-]{2,40}$/.test(b.source_json.district)
        ? b.source_json.district
        : null,
...
function nearestAlternatives(slots: any[], startIso: string): string[] {
  ...
  return flatten(slots)
    ...
    .sort(
      (a, b2) =>
        Math.abs(Date.parse(a) - target) - Math.abs(Date.parse(b2) - target),
    )
    .slice(0, 3);
}
...
async function mapSpeciesId(
  ...
  try {
    ...
    const entry = Object.entries(aliases).find(([, names]) =>
      names.includes(s),
    );
    ...
    const match = list.find((sp: any) => {
      const names = [sp?.species, sp?.label, sp?.name]
        .filter((v) => v != null)
...
async function defaultVetId(clinicDigitailId: number): Promise<string> {
  const data = await digitailRequest(
    "GET",
    `/vets?filter%5Bclinic_id%5D=${clinicDigitailId}`,
  );
...
async function runSweep() {
  try {
    const lock = await pool.query(`SELECT pg_try_advisory_lock($1) AS got`, [
      ADVISORY_LOCK_KEY,
    ]);
    ...
    try {
      ...
      await pool.query(
        `DELETE FROM hub_otp_codes WHERE created_at < NOW() - interval '24 hours'`,
      );
...
export async function runConfirmPipeline(
  ...
  if (claim.rowCount === 0) {
    ...
    if (cur?.status === "confirmed" && cur?.digitail_appointment_id) {
      return {
        outcome: "confirmed",
        ref: referenceCode(cur.brand, cur.id),
        idempotent: true,
      };
    }
  ...
  if (b.hold_expires_at && new Date(b.hold_expires_at).getTime() < Date.now()) {
    await pool.query(
      `UPDATE hub_bookings SET status = 'expired', updated_at = NOW() WHERE id = $1`,
      [b.id],
    );
  ...
  const day = new Date(svc.starts_at)
    .toLocaleString("sv-SE", { timeZone: clinic?.timezone || "Asia/Riyadh" })
    .slice(0, 10);
  const slug = (clinic?.config_json || {}).slug;
  ...
  const slotClaim = await pool.query(
    ...
    [
      b.clinic_id,
      String(svc.service_id || "0"),
      new Date(svc.starts_at).toISOString(),
      b.id,
    ],
  );
  if (slotClaim.rowCount === 0) {
    ...
    if (slug) {
      try {
        alternatives = nearestAlternatives(
          await fetchDaySlots(
            slug,
            day,
            svc.service_id,
    ...
    await pool.query(
      `UPDATE hub_bookings SET status = 'failed', updated_at = NOW() WHERE id = $1`,
      [b.id],
    );
    await audit("system", "booking.conflict", "hub_bookings", b.id, {
      reason: "slot_claim_contended",
      clinic: b.clinic_id,
    });
  ...
  try {
    ...
    try {
      const vetId =
        svc.doctor_id || (await defaultVetId(clinic.digitail_clinic_id));
      const parentId = await findOrCreatePetParent(
        clinic.digitail_clinic_id,
        b.customer_name,
        b.customer_phone,
      );
      const petSnap = b.pet_snapshot_json || {};
      const speciesId = await mapSpeciesId(petSnap.species);
      const patientId = await createDigitailPet(
        clinic.digitail_clinic_id,
        vetId,
        parentId,
        petSnap,
      ...
      const appt = await digitailRequest("POST", "/appointments", {
        ...
        datetime_end_utc: new Date(
          svc.ends_at ||
            new Date(
              new Date(svc.starts_at).getTime() +
      ...
      await pool.query(
        `UPDATE hub_pets SET digitail_patient_id = $2, digitail_parent_id = $3, updated_at = NOW() WHERE id = $1`,
        [b.pet_id, patientId, parentId],
      );
      ...
      const vars = {
        ...
        date: new Date(svc.starts_at)
          .toLocaleString("sv-SE", { timeZone: "Asia/Riyadh" })
          .slice(0, 10),
        time: new Date(svc.starts_at)
          .toLocaleString("sv-SE", { timeZone: "Asia/Riyadh" })
          .slice(11, 16),
      };
      const templates = ((await getBrandRow(b.brand))?.config_json || {})
        .templates;
      await enqueueNotification({
        ...
        templateKey: "confirm",
        payload: { body: renderTemplate("confirm", vars, b.locale, templates) },
        scheduledAt: new Date(),
        respectQuietHours: false,
      });
      const startMs2 = new Date(svc.starts_at).getTime();
      for (const [kind, offsetMs] of [
        ["remind_24h", 24 * 3600_000],
        ["remind_2h", 2 * 3600_000],
      ] as const) {
        ...
        if (at.getTime() > Date.now() + 60_000) {
          await enqueueNotification({
            ...
            toPhone: b.customer_phone,
            templateKey: kind,
            payload: { body: renderTemplate(kind, vars, b.locale, templates) },
            scheduledAt: at,
          });
        }
      }
      await audit(actor, "booking.confirmed", "hub_bookings", b.id, {
        ref,
        appointment: apptId,
      });
      return {
      ...
      await pool.query(
        `UPDATE hub_bookings SET status = $2, updated_at = NOW() WHERE id = $1 AND status = 'confirming'`,
        [b.id, b.status],
      );
      await audit(
        "system",
        "booking.confirm_upstream_error",
        "hub_bookings",
        b.id,
        { status: e?.status || 0 },
      );
    ...
    await pool
      .query(
        `DELETE FROM hub_slot_claims WHERE clinic_id = $1 AND service_id = $2 AND slot_start = $3 AND booking_id = $4`,
        [
          b.clinic_id,
          String(svc.service_id || "0"),
          new Date(svc.starts_at).toISOString(),
...
export async function loadBookingById(id: string) {
  const r = await pool.query(
    `SELECT * FROM hub_bookings WHERE id = $1 LIMIT 1`,
    [id],
  );
  return r.rows[0] || null;
}
