...
import { registerCmsRoutes } from "./cms";
...
const hash = (p: string) =>
  crypto
    .createHash("sha256")
    .update(p + (process.env.ADMIN_SALT || "elite-onx"))
    .digest("hex");
...
export function registerAdminRoutes(app: Express) {
  if (!dbEnabled) {
    ...
    app.all(
      ["/api/bookings", "/api/messages", "/api/content", "/api/services"],
      (_req, res) => res.status(503).json({ error: "database_not_configured" }),
    );
    return;
  }
    ...
    if (existing.length === 0) {
      ...
      if (envPass && username === envUser && password === envPass) {
        const [created] = await db
          .insert(adminUsers)
          .values({ username: envUser, passwordHash: hash(envPass) })
          .returning();
  ...
  app.post("/api/bookings", async (req, res) => {
    ...
    if (serviceId) {
      ...
      if (found[0].isActive !== "true")
        return res.status(400).json({ error: "inactive_service" });
      svc = found[0];
      amount = svc.price;
    }
    ...
    await sendNotification(
      "حجز جديد — موقع النخبة",
      bookingEmailHtml({
        ...b,
        service: serviceName,
        paymentStatus: payment?.ok ? "بانتظار الدفع" : "غير مدفوع",
      }),
    );
    res.json({
      ok: true,
      bookingId: b.id,
      invoiceUrl: payment?.ok ? payment.invoiceUrl : null,
      paymentNotConfigured: !!(
        amount &&
        Number(amount) > 0 &&
        payment &&
        !payment.ok &&
        payment.reason === "myfatoorah_not_configured"
      ),
    });
  });
  ...
  app.get("/api/payment/callback", async (req, res) => {
      ...
      if (st.ok && st.customerReference) {
        ...
        ps =
          st.invoiceStatus === "Paid"
            ? "paid"
            : st.invoiceStatus === "Failed"
              ? "failed"
              : "pending";
        await db
          .update(bookings)
          .set({ paymentStatus: ps, paymentId })
        ...
        if (dup.length) {
          await db
            .update(payments)
            .set({ status: ps, updatedAt: new Date().toISOString() })
            .where(eq(payments.paymentId, paymentId));
        } else {
          const [bk] = await db
            .select()
          ...
          await db.insert(payments).values({
            bookingId,
            paymentId,
            invoiceId: st.invoiceId || null,
            amount: bk?.amount || null,
            currency: "SAR",
            status: ps,
          });
        }
    ...
    const target =
      ps === "paid" ? "success" : ps === "failed" ? "failed" : "pending";
    res.redirect(
      `/book-now?payment=${target}${bookingId ? `&booking=${bookingId}` : ""}`,
    );
  });
      ...
      if (st.ok && st.customerReference) {
        ...
        if (ps === "paid") {
          await db
            ...
            .where(eq(bookings.id, st.customerReference));
          return res.redirect(
            `/book-now?payment=success&booking=${st.customerReference}`,
          );
        }
        await db
          .update(bookings)
  ...
  app.post("/api/messages", async (req, res) => {
    ...
    await sendNotification(
      ...
      `<div dir=rtl><p><b>${name}</b> (${phone || ""} ${email || ""})</p><p>${body}</p></div>`,
    );
    res.json({ ok: true });
  });
  ...
  app.put("/api/admin/content/:key", requireAdmin, async (req, res) => {
    ...
    if (existing.length) {
      ...
      await db
        .insert(siteContent)
        .values({ key, valueAr, valueEn, type: type || "text", section });
    }
    res.json({ ok: true });
  });
  ...
  app.post("/api/admin/media", requireAdmin, async (req, res) => {
    ...
    if (!filename || !dataBase64)
      return res
        .status(400)
        .json({ error: "filename and dataBase64 required" });
    if (dataBase64.length > 2_500_000)
      return res.status(400).json({ error: "file too large (max ~1.8MB)" });
    const url = `data:${mimeType || "image/png"};base64,${dataBase64}`;
    const [m] = await db
      .insert(mediaFiles)
      .values({ filename, url, mimeType, size: String(dataBase64.length) })
      .returning();
